...
function makeAjaxRequestWithRetriesAndJitter(url, method, data, successCallback, errorCallback) {
  console.log("makeAjaxRequestWithRetriesAndJitter");
  //   var MAX_RETRIES = 4;
...
function getStyles(boxColor, primaryTextColor, primaryColor) {
  return (
    "#main_invite_div {background-color:" +
    boxColor +
    "; color:" +
    primaryTextColor +
...
function getCodeScreen(codeInfo, codePlaceholder, submitText, verifyingMessage) {
  return (
    '<form onsubmit="verifyCode(event)"><div id="main_invite_div">' +
...
function getCustomCodeScreen(codePlaceholder, submitText, verifyingMessage) {
  return (
    '<form onsubmit="verifyCustomCode(event)"><div id="main_invite_div">' +
...
if (window.queueViewModel.queuePathPrefix) {
  ...
  queuePathPrefix = queuePathPrefix[0] === "/" ? queuePathPrefix : "/" + queuePathPrefix;
}
...
function renderHtml(containerId, parameters) {
  ...
  inviteOnlyEmailVerificationCallback = parameters.InviteOnlyEmailVerificationCallback;
...
function checkPermissionCollectionEnabledOnEvent() {
  console.log("checkPermissionCollectionEnabledOnEvent");
  var emailFromUrl = getParameterByName("email");
  $("#error_box").html("");
  makeAjaxRequestWithRetriesAndJitter(
    baseURL + "/check-permission-collection",
    "POST",
    JSON.stringify({
    ...
    function (data) {
      ...
      if (emailFromUrl != "" && emailFromUrl != undefined) {
        document.getElementById("invitee_email").value = emailFromUrl;
...
function verifyEmailGetCode(event) {
  ...
  $("#error_box").html("");
  makeAjaxRequestWithRetriesAndJitter(
    baseURL + (isSignupCase ? "/send-email" : "/check-invite"),
    "POST",
    JSON.stringify({
    ...
    function (data) {
      ...
      if (data.enqueueToken !== null && data.enqueueToken != undefined) {
        var a = {
          type: "success",
          sessionId: eventId,
...
function verifyCode(event) {
  event.preventDefault();
  var code = document.getElementById("invitee_code").value;
  $("#loading_div_code").show();
  $("#error_box_code").html("");
  makeAjaxRequestWithRetriesAndJitter(
    baseURL + "/verify-identifier",
    "POST",
    JSON.stringify({
      identifierValue: identifierValue,
      waitingRoomId: eventId,
...
function checkCustomCodeEnabledForCustomer(containerId) {
  ...
  makeAjaxRequestWithRetriesAndJitter(
    ...
    function (data) {
      ...
      if (isCustomCode == true && skipUiValidation == true && customCodeFromUrl != "" && customCodeFromUrl != undefined) {
...
function verifyCustomCode(event) {
  ...
  makeAjaxRequestWithRetriesAndJitter(
    ...
    function (data) {
      $("#loading_div").hide();
      if (data.success && data.enqueueToken !== null && data.enqueueToken != undefined) {
        var a = {
          type: "success",
          sessionId: eventId,
          challengeDetails: "InviteOnlyEmailVerification",
